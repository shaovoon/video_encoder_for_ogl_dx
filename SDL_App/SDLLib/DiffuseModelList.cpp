...
namespace Library
{
	RTTI_DEFINITIONS(DiffuseModelList)

		DiffuseModelList::DiffuseModelList(Scene& game, Camera& camera, const std::string& srcImageFile, const std::vector<std::string>& srcObjFiles, const std::string& srcMtlFile)
    ...
    {
		for (size_t i = 0; i < srcObjFiles.size(); ++i)
		{
			mModelVertices.push_back(ModelVertices(srcObjFiles[i]));
		}
		DownloadFiles();
    }

	DiffuseModelList::~DiffuseModelList()
	{
	...
	void DiffuseModelList::Initialize()
	{
		LOGFUNCTION("DiffuseModelList", "Initialize");

		for (size_t i = 0; i < mModelVertices.size(); ++i)
		{
			ModelVertices& m = mModelVertices[i];
			std::string obj_file = m.mDestObjFile;
			if (m.mDestObjFile.find(".gz") == m.mDestObjFile.size() - 3) // gz compressed file
			{
				obj_file = obj_file.substr(0, obj_file.size() - 3);
			}
			if (m.mDestObjFile.find(".zip") == m.mDestObjFile.size() - 4) // gz compressed file, ignore the zip ext.
			{
...
#endif
			{
				Utility::DecompressFile(m.mDestObjFile.c_str(), obj_file.c_str());
			}
...
void main()
{
	...
	void DiffuseModelList::UpdatePosition(const SceneTime& gameTime, glm::mat4x4& mat)
	{
	...
	void DiffuseModelList::Draw(const SceneTime& gameTime, size_t index)
	{
	...
	void DiffuseModelList::Draw(const SceneTime& gameTime)
	{
		Draw(gameTime, 0);
	}

	void DiffuseModelList::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer) const
	{
	...
	void DiffuseModelList::CreateVertexBuffer(VertFormat* vertices, GLuint vertexCount, GLuint& vertexBuffer) const
	{
	...
	size_t DiffuseModelList::VertexSize() const
    {
		return sizeof(VertFormat);
    }

	void DiffuseModelList::DownloadFiles()
	{
		...
		for (size_t i = 0; i < mModelVertices.size(); ++i)
		{
			...
			gDownloadSingleton.DownloadFile(this, srcFolder, m.mSrcObjFile,
	...
	bool DiffuseModelList::OpenFile(const char* file, FileType file_type)
	{
		...
		if (file_type == Library::DownloadableComponent::FileType::OBJ_FILE)
		{
			for (size_t i = 0; i < mModelVertices.size(); ++i)
			{
				ModelVertices& m = mModelVertices[i];
				m.mFile = file;
				if (m.mFile.find(m.mSrcObjFile) != std::string::npos)
				{
					m.mDestObjFile = file;
					return true;
				}
