...
namespace Library
{
	RTTI_DEFINITIONS(EarthGlobe)

	EarthGlobe::EarthGlobe(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile)
	...
	{
		LOGFUNCTION("EarthGlobe", "Initialize");
...
void main()
{
	...
	void EarthGlobe::UpdatePosition(const SceneTime& gameTime, glm::mat4x4& mat)
	{
	...
	void EarthGlobe::Draw(const SceneTime& gameTime)
	{
	...
	void EarthGlobe::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer) const
	{
		...
		for (size_t i = 0; i < sourceVertices.size(); i++)
		{
			...
			vec2 uv = vec2(-textureCoordinates[i].x, textureCoordinates[i].y);
	...
	void EarthGlobe::CreateVertexBuffer(VertFormat* vertices, GLuint vertexCount, GLuint& vertexBuffer) const
	{
	...
	size_t EarthGlobe::VertexSize() const
    {
		return sizeof(VertFormat);
    }

	void EarthGlobe::DownloadFiles()
	{
	...
	bool EarthGlobe::OpenFile(const char* file, FileType file_type)
	{
