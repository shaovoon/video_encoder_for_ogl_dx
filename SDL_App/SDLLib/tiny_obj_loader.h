...
bool LoadObj(std::vector<shape_t> &shapes,       // [output]
             std::vector<material_t> &materials, // [output]
             std::string& err,                   // [output]
             FILE* fileStream, MaterialReader &readMatFn);
...
bool MaterialFileReader::operator()(const std::string &matId,
  ...
  if (!m_mtlBasePath.empty()) {
	  filepath = std::string(m_mtlBasePath) + matId + ".txt";
  } else {
    filepath = matId + ".txt";
  }
  ...
  fopen_s(&matFile, filepath.c_str(), "rt");
#else
  FILE *matFile = fopen(filepath.c_str(), "rt");
#endif
  if (!matFile)
  {
	  char buf[1000];
#ifdef _MSC_VER
	  sprintf_s(buf, "WARN: Material file [ %s ] not found. Created a default material.\n", filepath.c_str());
#else
	  sprintf(buf, "WARN: Material file [ %s ] not found. Created a default material.\n", filepath.c_str());
#endif
  ...
  LoadMtl(matMap, materials, matFile);
  fclose(matFile);
  return true;
}
...
#ifdef _MSC_VER
  FILE *ifs = nullptr;
  fopen_s(&ifs, filename, "rt");
#else
  FILE *ifs = fopen(filename, "rt");
#endif
  if (!ifs)
  {
	char buf[1000];
