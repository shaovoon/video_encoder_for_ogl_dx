...
namespace Library
{
	class BinaryModel : public DrawableSceneComponent
    {
		RTTI_DECLARATIONS(BinaryModel, DrawableSceneComponent)
		typedef VertexPositionTextureNormal VertFormat;

    public:
		BinaryModel(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile);
		~BinaryModel();
