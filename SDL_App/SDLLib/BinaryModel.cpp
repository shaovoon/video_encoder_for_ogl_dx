...
namespace Library
{
	RTTI_DEFINITIONS(BinaryModel)

	BinaryModel::BinaryModel(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile)
	...
	void BinaryModel::Initialize()
	{
		LOGFUNCTION("BinaryModel", "Initialize");
...
void main()
{
	...
	if(diff > 0.5)
		diffuse = 0.6 * s_LightColor;
	else 
		diffuse = diff * s_LightColor;
	...
	void BinaryModel::UpdatePosition(const SceneTime& gameTime, glm::mat4x4& mat)
	{
	...
	void BinaryModel::Draw(const SceneTime& gameTime)
	{
	...
	void BinaryModel::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer) const
	{
	...
	void BinaryModel::CreateVertexBuffer(VertFormat* vertices, GLuint vertexCount, GLuint& vertexBuffer) const
	{
	...
	size_t BinaryModel::VertexSize() const
    {
		return sizeof(VertFormat);
    }

	void BinaryModel::DownloadFiles()
	{
	...
	bool BinaryModel::OpenFile(const char* file, FileType file_type)
	{
