...
namespace Library
{
	class DiffuseModel : public DrawableSceneComponent
    {
		RTTI_DECLARATIONS(DiffuseModel, DrawableSceneComponent)
		typedef VertexPositionTextureNormal VertFormat;

    public:
		DiffuseModel(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile);
		~DiffuseModel();
