...
namespace Library
{
	RTTI_DEFINITIONS(SpecularModel)

	SpecularModel::SpecularModel(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile)
	: DrawableSceneComponent(game, camera), mShaderProgram(),
		...
		mVboId(-1), mIndexId(-1), mIndexCount(0), mLightColor(1.0f, 1.0f, 1.0f), mLightPosition(1.0f,0.0f,1.0f),
	...
	void SpecularModel::Initialize()
	{
		LOGFUNCTION("SpecularModel", "Initialize");
...
void main()
{
	...
	/*
	void SpecularModel::UpdatePosition(const SceneTime& gameTime, glm::mat4x4& mat)
	{
	...
	*/
	...
	void SpecularModel::Draw(const SceneTime& gameTime)
	{
	...
	void SpecularModel::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer) const
	{
	...
	void SpecularModel::CreateVertexBuffer(VertFormat* vertices, GLuint vertexCount, GLuint& vertexBuffer) const
	{
	...
	size_t SpecularModel::VertexSize() const
    {
		return sizeof(VertFormat);
    }

	void SpecularModel::DownloadFiles()
	{
	...
	bool SpecularModel::OpenFile(const char* file, FileType file_type)
	{
