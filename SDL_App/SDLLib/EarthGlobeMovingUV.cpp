...
namespace Library
{
	RTTI_DEFINITIONS(EarthGlobeMovingUV)

	EarthGlobeMovingUV::EarthGlobeMovingUV(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile)
	: DrawableSceneComponent(game, camera), mShaderProgram(),
		...
		mVboId(-1), mIndexId(-1), mIndexCount(0), mLightColor(1.0f, 1.0f, 1.0f), mLightPosition(1.0f,0.5f,0.7f),
	...
	EarthGlobeMovingUV::~EarthGlobeMovingUV()
	{
	...
	void EarthGlobeMovingUV::Initialize()
	{
		LOGFUNCTION("EarthGlobeMovingUV", "Initialize");
...
void main()
{
	...
	void EarthGlobeMovingUV::UpdatePosition(const SceneTime& gameTime, glm::mat4x4& mat)
	{
		static GLfloat y_angle = 0.0f;
		const GLfloat x_angle = static_cast<GLfloat>(M_PI);
		const GLfloat z_tilt = 15.0f / 180.0f * static_cast<GLfloat>(M_PI);
		mat = glm::rotate(mat, z_tilt, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	void EarthGlobeMovingUV::Draw(const SceneTime& gameTime)
	{
	...
	void EarthGlobeMovingUV::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer) const
	{
	...
	void EarthGlobeMovingUV::CreateVertexBuffer(VertFormat* vertices, GLuint vertexCount, GLuint& vertexBuffer) const
	{
	...
	size_t EarthGlobeMovingUV::VertexSize() const
    {
		return sizeof(VertFormat);
    }

	void EarthGlobeMovingUV::DownloadFiles()
	{
	...
	bool EarthGlobeMovingUV::OpenFile(const char* file, FileType file_type)
	{
