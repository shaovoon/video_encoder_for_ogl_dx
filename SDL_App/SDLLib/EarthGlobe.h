...
namespace Library
{
	class EarthGlobe : public DrawableSceneComponent
    {
		RTTI_DECLARATIONS(EarthGlobe, DrawableSceneComponent)
		typedef VertexPositionTextureNormal VertFormat;

    public:
		EarthGlobe(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile);
		~EarthGlobe();
