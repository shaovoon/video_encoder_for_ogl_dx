...
namespace Library
{
	class EarthGlobeMovingUV : public DrawableSceneComponent
    {
		RTTI_DECLARATIONS(EarthGlobeMovingUV, DrawableSceneComponent)
		typedef VertexPositionTextureNormal VertFormat;

    public:
		EarthGlobeMovingUV(Scene& game, Camera& camera, const std::string& srcImageFile, const std::string& srcObjFile, const std::string& srcMtlFile);
		~EarthGlobeMovingUV();
